...
class DomainException(InterviewIQException):
    def __init__(
        self, message: str, code: str = "DOMAIN_ERROR", details: Optional[Any] = None
    ):
...
class UnauthorizedException(InterviewIQException):
    def __init__(
        self, message: str = "Unauthorized operation", details: Optional[Any] = None
    ):
...
class StateTransitionException(DomainException):
    def __init__(self, current_state: str, action: str):
        message = (
            f"Invalid state transition: Cannot perform action '{action}' "
...
class AIProviderException(InterviewIQException):
    def __init__(
        ...
        super().__init__(
            ...
            details={"provider": provider, "extra": details},
        )
