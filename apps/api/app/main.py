...
async def interviewiq_exception_handler(request: Request, exc: InterviewIQException):
    logger.error(f"InterviewIQException [{exc.code}]: {exc.message} - Details: {exc.details}")
    status_code = 400
...
app.include_router(identity_router, prefix=f"{v1_prefix}/identity", tags=["1. Identity & Access"])
app.include_router(organizations_router, prefix=f"{v1_prefix}/organizations", tags=["2. Organizations & Membership"])
app.include_router(candidates_router, prefix=f"{v1_prefix}/candidates", tags=["3. Candidates"])
app.include_router(resumes_router, prefix=f"{v1_prefix}/resumes", tags=["4. Resumes"])
app.include_router(job_roles_router, prefix=f"{v1_prefix}/job-roles", tags=["5. Job Roles"])
app.include_router(knowledge_rag_router, prefix=f"{v1_prefix}/knowledge-rag", tags=["6. Knowledge & RAG Retrieval"])
app.include_router(interviews_router, prefix=f"{v1_prefix}/interviews", tags=["7. Interview Orchestration"])
app.include_router(interview_intelligence_router, prefix=f"{v1_prefix}/intelligence", tags=["8. Interview Intelligence"])
app.include_router(reports_router, prefix=f"{v1_prefix}/reports", tags=["9. Reports"])
app.include_router(background_jobs_router, prefix=f"{v1_prefix}/jobs", tags=["10. Background Jobs"])
app.include_router(audit_logging_router, prefix=f"{v1_prefix}/audit", tags=["11. Audit Logging"])
app.include_router(shared_router, prefix=f"{v1_prefix}/shared", tags=["12. Shared Infrastructure"])
